...
class ManagerAssignGigController extends Controller
{
    ...
    public function index()
    {
        ...
        // Fetch all assigned shifts where the assignee has the same location_id
        ...
        if ($assign_gig->isEmpty()) {
            return response()->json(['status'=>200,'response'=>'Not Found','message'=>'Assigned shift(s) does not exist', 'data' => $assign_gig], 200);
        }
        ActivityLog::create([
            'action' => 'View All shifts assign',
            'description' => auth()->user()->last_name.' '.auth()->user()->first_name.' viewed all shifts assign at '.Carbon::now()->format('h:i:s A'),
            'subject_id' => auth()->id(),
            'subject_type' => get_class($assign_gig ),
            'user_id' => auth()->id(),
        ]);
        return response()->json(['status'=>200,'response'=>'Successful',"message"=>"All Assigned shift(s) fetched successfully","data"=>$assign_gig ],200);
    }
    ...
    public function store(Request $request)
    {
                ...
                foreach ($existingSchedule as $existingShift) {
                    if ($currentShift['day'] == $existingShift['day'] &&
                        ...
                        return response()->json([
                            'status' => 409,
                            'response' => 'Conflict',
                            'message' => 'User is already assigned to a shift with the same time shift.'
                        ], 409);
                    }
                }
            }
        ...
        return response()->json(['status' => 201, 'response' => 'Assigned shift', 'message' => 'Shift assigned successfully', 'data' => [$assigned_gig]], 201);
    }
    /**
    ...
    public function show(Request $request)
    {
        ...
        if (is_null($assign_gig)) {
            return response()->json(['status' => 404, 'response' => 'Not Found', 'message' => 'Assigned shift does not exist'], 404);
        }
    
        ActivityLog::create([
            'action' => 'View Assigned shift',
            'description' => auth()->user()->last_name . ' ' . auth()->user()->first_name . ' viewed an assigned shift at ' . Carbon::now()->format('h:i:s A'),
            'subject_id' => auth()->id(),
            'subject_type' => AssignGig::class,
            'user_id' => auth()->id(),
        ]);
    
        return response()->json(['status' => 200, 'response' => 'Successful', 'message' => 'Assigned shift fetched successfully', 'data' => $assign_gig], 200);
    }
    /**
    ...
    public function update(Request $request)
    {
        ...
        // Check if the user has already been assigned to a shift with the same time shift, excluding the current assignment
        $existingAssignments = AssignGig::where('user_id', $request->user_id)
            ->where('id', '!=', $request->id)
            ->with('schedule')
            ->get();
        ...
        ActivityLog::create([
            'action' => 'shift has been assigned to ' . $user->last_name . ' ' . $user->first_name . ' was updated',
            'description' => auth()->user()->last_name . ' ' . auth()->user()->first_name . ' updated shift assigned to ' . $user->last_name . ' ' . $user->first_name . ' at ' . Carbon::now()->format('h:i:s A'),
            'subject_id' => $assign_gig->id,
            'subject_type' => get_class($assign_gig),
            'user_id' => auth()->id(),
        ]);

        return response()->json(['status' => 200, 'response' => 'Updated Assigned shift', 'message' => 'Assigned shift updated successfully', 'data' => [$assigned_gig]], 200);
    }
    /**
    ...
    public function destroy(Request $request)
    {
        ...
        $gig->delete();
        ActivityLog::create([
            'action' => 'Deleted Assigned shift',
            'description' => auth()->user()->last_name.' '.auth()->user()->first_name.' deleted assigned shift at '.Carbon::now()->format('h:i:s A'),
            'subject_type' => get_class($gig),
            'subject_id' => $request->id,
            'user_id' => auth()->id(),
        ]);
        return response()->json(['status'=>204,'response'=>'Not Content','message' => 'Assigned shift Deleted successfully'],200);
    }

...
class SupervisorGigController extends Controller
{
    ...
    /*public function index()
    {
        ...
        $gigs = Gig::with(['client','schedule','supervisor','assignments.assignee','assignments.assignee.supervisor_in_charges'])
    ...
    public function index()
    {
    // Get the authenticated user's ID (assuming this is the supervisor ID)
    $userId = auth()->id();
    $userLocationId = auth()->user()->location_id;

    // Fetch all gigs where the logged-in user is the supervisor in charge
    $gigs = Gig::with(['client', 'schedule', 'assignments.assignee.supervisor_in_charges'])
        ...
        ->whereNotIn('status', ['ended', 'completed']) // Exclude gigs that are ended or completed
        ->latest()
        ->get();
    ...
    if ($gigs->isEmpty()) {
        return response()->json([
            'status' => 200,
            'response' => 'Not Found',
            'message' => 'Gig(s) does not exist',
            'data' => []
        ], 200);
    }

    // Transform the gigs data to replace 'supervisor' with the supervisor details from the users table
    $gigsTransformed = $gigs->map(function ($gig) {
        // Assuming only one supervisor per assignee in assignments
        $gig->supervisor = $gig->assignments->map(function ($assignment) {
            // Check if supervisor_in_charges exists and is a collection
            $supervisorInCharges = $assignment->assignee->supervisor_in_charges;

            if ($supervisorInCharges->isNotEmpty()) {
                // Get the first supervisor from the collection
                $supervisorId = $supervisorInCharges->first()->supervisor_id ?? null;

                // Query the User model to get supervisor details
                if ($supervisorId) {
                    return User::find($supervisorId); // Return full supervisor details
                }
            }

            return null;
        })->first(); // Assuming only one supervisor in charge

        return $gig;
    });

    // Log the activity of viewing gigs
    ActivityLog::create([
        'action' => 'View All Gigs',
        'description' => auth()->user()->last_name . ' ' . auth()->user()->first_name . ' viewed all gigs at ' . Carbon::now()->format('h:i:s A'),
        'subject_id' => auth()->id(),

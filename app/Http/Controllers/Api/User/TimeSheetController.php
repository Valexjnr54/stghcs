...
class TimeSheetController extends Controller
{
   ...
   /* if ($schedule_time['status'] === 404) {
    ...
    {
        $time_sheet = TimeSheet::where('unique_id', $request->timesheet_id)->first();
        $gig_id = $time_sheet->gig_id;
        $user_id  = $time_sheet->user_id;
        
        // Retrieve assign_gig
        $assign_gig = AssignGig::with(['gig.client', 'schedule', 'assignee'])
            ->where(['user_id' => $user_id, 'gig_id' => $gig_id])
            ->first();
        
        if(is_null($assign_gig->gig->client) || $assign_gig->gig->client->status != "active")
        {
            return response()->json([
                'status' => 404,
                'response' => 'Inactive client',
